...
class SOAP(BaseOptimizer):
    ...
    def project(
        ...
        for mat in state['Q']:
            if len(mat) > 0:
                # Convert matrix to grad's dtype for the operation
                mat_cast = mat.to(grad_dtype)
                grad = torch.tensordot(grad, mat_cast, dims=[[0], [0 if project_type == 'forward' else 1]])
            else:
    ...
    def get_orthogonal_matrix_qr(self, state, max_precondition_dim: int = 10000, merge_dims: bool = False):
        ...
        exp_avg_sq_dtype = state['exp_avg_sq'].dtype
        ...
        for ind, (m, o) in enumerate(zip(state['GG'], state['Q'])):
            ...
            # Convert to exp_avg_sq's dtype for consistent computation
            m = m.to(exp_avg_sq_dtype)
            o = o.to(exp_avg_sq_dtype)
    ...
    def init_pre_conditioner(
        ...
        if grad.dim() == 1:
            if not precondition_1d or grad.shape[0] > max_precondition_dim:
                ...
                # Initialize with float32 for numerical stability
    ...
    def update_pre_conditioner(
        ...
        if grad.dim() == 1:
            if precondition_1d and grad.shape[0] <= max_precondition_dim:
                ...
                outer_product = (grad_float32.unsqueeze(1) @ grad_float32.unsqueeze(0))
                state['GG'][0].lerp_(outer_product, weight=1.0 - state['shampoo_beta'])
        else:
            if merge_dims:
                grad = grad.reshape(merge_small_dims(grad.size(), max_precondition_dim))
    ...
    def step(self, closure: CLOSURE = None) -> LOSS:
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # Keep optimizer states in float32 for stability
                exp_avg, exp_avg_sq = state['exp_avg'].to(torch.float32), state['exp_avg_sq'].to(torch.float32)

                grad_projected = self.project(
                    grad, state, merge_dims=group['merge_dims'], max_precondition_dim=group['max_precondition_dim']
                )

                exp_avg.mul_(beta1).add_(grad.to(torch.float32), alpha=1.0 - beta1)
                exp_avg_sq.mul_(beta2).add_(grad_projected.to(torch.float32).square(), alpha=1.0 - beta2)

                de_nom = exp_avg_sq.sqrt().add_(group['eps'])

                exp_avg_projected = self.project(
                    exp_avg.to(grad.dtype), state, merge_dims=group['merge_dims'],
                    max_precondition_dim=group['max_precondition_dim']
                )
                ...
                norm_grad = self.project(
                    (exp_avg_projected / de_nom.to(grad.dtype)),
                    state,
                    merge_dims=group['merge_dims'],
                    max_precondition_dim=group['max_precondition_dim'],
