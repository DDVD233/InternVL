...
class CustomGRPOTrainer(GRPOTrainer):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None):
        # Override evaluation to check answers within <answer> tags
        ...
        # Add custom metrics based on answer extraction
        predictions = []
        references = []

        for output in eval_results.predictions:
            pred_answer = extract_answer(output)
            if pred_answer:
                predictions.append(pred_answer)

        for ref in eval_results.label_ids:
        ...
        eval_results.metrics["answer_accuracy"] = accuracy
        return eval_results


def format_reward_func(completions: List[str], ground_truth: List[str], **kwargs) -> List[float]:
    """
    Compute rewards based on format, thinking length, and answer accuracy.
    ...
    for completion, truth in zip(completions, ground_truth):
        ...
        # 1. Check presence of both blocks (reward: 1.0)
        ...
        if think_match and answer_match:
            ...
            # 2. Check thinking length (reward: up to 2.0)
            think_content = think_match.group(1).strip()
            think_length = len(think_content)
            # Linearly increase reward up to 200 characters, cap at 2.0
            length_reward = min(think_length / 100, 2.0)
            reward += length_reward

            # 3. Check answer accuracy (reward: 3.0)
            answer_content = answer_match.group(1).strip().lower()
            # Extract ground truth from reference if it's in the same format
            truth_match = re.search(answer_pattern, truth, re.DOTALL)
            if truth_match:
                truth_content = truth_match.group(1).strip().lower()
            else:
                truth_content = truth.strip().lower()

            if answer_content == truth_content:
                reward += 3.0
...
def add_clinical_instruction(conversation):
    """Add clinical instruction to the conversation"""
    instruction = ("You are an expert radiologist. You will be presented a clinical diagnosis question. "
                   "Before answer your question, first think about how can you make the diagnosis using "
                   "your clinical knowledge. Wrap your thinking within <think> ... </think> block. Then, "
                   "provide the answer in a word or phrase in <answer> ... </answer> block.")
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            try:
                ...
                data_item['conversations'] = add_clinical_instruction(data_item['conversations'])
...
def main():
    ...
    training_args = GRPOConfig(**{
        # Copy all attributes from training_args
    ...
    optimizer = PrecondScheduleForeachSOAP(model.parameters(), lr=training_args.learning_rate, betas=(0.9, 0.98),
